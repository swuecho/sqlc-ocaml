...
func TestGenerateMVP(t *testing.T) {
	...
	if got, want := strings.Count(ml, "  let operation = "), len(request().Queries); got != want {
		t.Errorf("generated %d operation bindings, want one for each of %d query modules", got, want)
	}
